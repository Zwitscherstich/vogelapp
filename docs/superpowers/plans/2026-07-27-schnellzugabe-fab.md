...
```ts
  ...
  for (const n of nachtraege) {
    try {
      ...
      // in der Warteschlange. Sonst wuerde ein Verbindungswackler beim
      // Wiederverbinden die Beobachtung des Nutzers stillschweigend loeschen.
      ...
      // entstuende sonst eine doppelte Verknuepfung. Der 23505-Handler unten
      // greift nur, WENN ein Constraint existiert -- was der Plan bewusst
      // nicht voraussetzt.
...
export default function SchnellZugabeFab() {
  ...
  // vollstaendige paginierte Abfrage aus, und zwar auf jeder Seite, weil der
  // FAB im Root-Layout haengt.
  useEffect(() => {
    if (!online) return;
    let abgebrochen = false;
